...
export class ApiStack extends Stack {
  constructor(scope: Construct, id: string, props: ApiStackProps) {
    ...
    const cloudWatchPolicy = new iam.Policy(this, "TieredAPI_CloudWatchLogsPolicy", {
      statements: [
    ...
    const lambdaCreateRole = new iam.Role(this, "TieredAPI_Create_LambdaRole", {
      ...
      inlinePolicies: { allowDDBCreateAndKMSPolicy: allowDDBCreateAndKMSPolicy.document, cloudWatchPolicy: cloudWatchPolicy.document }
    })

    const lambdaReadRole = new iam.Role(this, "TieredAPI_Read_LambdaRole", {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      description: 'Assumed by Lambda Functions',
      inlinePolicies: { allowDDBReadsAndKMSPolicy: allowDDBReadsAndKMSPolicy.document, cloudWatchPolicy: cloudWatchPolicy.document }
    })

    const lambdaUpdateRole = new iam.Role(this, "TieredAPI_Update_LambdaRole", {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      description: 'Assumed by Lambda Functions',
      inlinePolicies: { allowDDBUpdateAndKMSPolicy: allowDDBUpdateAndKMSPolicy.document, cloudWatchPolicy: cloudWatchPolicy.document }
    })

    const lambdaDeleteRole = new iam.Role(this, "TieredAPI_Delete_LambdaRole", {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      description: 'Assumed by Lambda Functions',
      inlinePolicies: { allowDDBDeleteAndKMSPolicy: allowDDBDeleteAndKMSPolicy.document, cloudWatchPolicy: cloudWatchPolicy.document }
    })

    const apiGatewayCloudWatchRole = new iam.Role(this, "TieredAPI_CloudWatchRole", {
